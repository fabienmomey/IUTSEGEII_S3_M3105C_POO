...
*/
void mammifere_ongule::se_nourrir(float quantite_nourriture)
{
    cout << endl << m_nom << " se nourrit de " << quantite_nourriture << "kg de nourriture." << endl << endl;
    m_poids += fminf(0.2*quantite_nourriture,0.1*m_poids);
    m_taille += fminf(0.2*quantite_nourriture,0.1*m_taille);
}
...
bool mammifere_ongule::fuir() const
{
    ...
    if (rand()%2)
    {
        cout << endl << m_nom << " a réussi à fuir." << endl << endl;
        return true;
    }
    else
    {
